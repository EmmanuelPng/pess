...
<body>
	...
	if (isset($_POST["btnDispatch"])){
		...
		$patrolcarDispatched = $_POST["chkPatrolcar"]; //array of patrol car being dispatched from post back
		$numberOfPatrolcarDispatched = count ($patrolcarDispatched);
		
		//insert new incident
		$incidentStatus;
		if ($numberOfPatrolcarDispatched > 0) {
			$incidentStatus ='2'; //incident status to be set as dispathed
		} else {
	...
	<!--display incident information passed from logcall.php-->
	...
	$mysqli = mysqli_connect(DB_SERVER, DB_USER, DB_PASSWORD, DB_DATABASE);
				...
				//retrieve from patrol car table those patrol cars that are 2:partol or 3:Free
