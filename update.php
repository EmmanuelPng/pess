...
<title>PESS</title>
	...
	if ($_POST["patrolCarStatus"] == 4){
	...
	} else if ($_POST["patrolCarStatus"] == 3){//else if patrol car status is FREE (3) them capture the tmie of completion
		
		//First, retroeve the incident ID from dispatch table handled by that patrolcar
		$sql = "SELECT incidentId FROM dispatch WHERE 
		timeCompleted IS NULL AND patrolcarId = ?";
		...
		//last but not least, update the incident table to complete (3) all patrol car attended to it are FREE now
		
		$sql = "UPDATE incident SET incidentStatusId = '3' WHERE 
		incidentId = '$incidentId' AND NOT EXISTS (SELECT * FROM dispatch WHERE timeCompleted IS NULL AND incidentId = '$incidentId')";
...
<body> <?php require_once 'nav.php'; ?><br><br>
	...
	<form name="form1" method="post" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>">
		<table width="50%" border="0" align="center" cellpadding="4" cellspacing="4">
			 ...
			 <td>Patrol Car ID:</td>
				<td><input type="text" name="patrolCarId" id="patrolCarId">
	...
	<?php }
		...
		// post back here after clicking the brn search button
		require_once 'db.php';
		...
		//retriece patrol car details
		$sql = "SELECT * FROM patrolcar WHERE patrolCarId = ?"; //was patrolcarId
		...
		if ($resultset->num_rows == 0){
			?>
	<script>windows.location="update.php";</script> 
		<?php }
	//else if the patrol car found
		...
		//retrieve from patrolcar_status table for populaing the combo box
